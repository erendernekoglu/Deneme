...
function startOfNextWeekMonday(): Date {
  const now = new Date();
...
async function main() {
  ...
  await prisma.employee.upsert({
    ...
    update: { passwordHash: adminPasswordHash } as any,
    create: {
  ...
  // Ensure passwords for demo employees if missing
  const ensurePass = async (email: string, pass: string) => {
    const u = await prisma.employee.findUnique({ where: { email } });
    const anyU = u as any;
    if (u && (!('passwordHash' in anyU) || !anyU.passwordHash)) {
      const hash = await bcrypt.hash(pass, 10);
      await prisma.employee.update({ where: { email }, data: { passwordHash: hash } as any });
    }
  };
  await ensurePass('alice@example.com', 'alice123');
  await ensurePass('bob@example.com', 'bob123');
